...
function EventInbox({ state, setState, update, setModal }: { state: TeamGameState; setState: React.Dispatch<React.SetStateAction<TeamGameState>>; update: UpdateTeamState; setModal: (value: string) => void }) {
  ...
  return <><h1>EVENT INBOX / รับเหตุการณ์</h1><Panel><div className="event-entry"><input placeholder="E03" value={code} maxLength={3} onChange={(event) => setCode(event.target.value.toUpperCase())} /><PixelButton onClick={submit}>RECEIVE EVENT / รับเหตุการณ์</PixelButton></div>{message && <p className="warning">{message}</p>}</Panel>
