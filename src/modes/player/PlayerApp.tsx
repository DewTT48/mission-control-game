...
import { getAllocatedEffort, getAvailableHours, getBudgetBreakdown, getDependencyPlanIssues, getEffectiveEffort, getSameDayDependencyHandoffs, getSelectedBudgetOption, getSummary, getTaskBudgetCost, getTaskPlannedFinishDay, getTaskPlannedStartDay, getTaskPlanStatus, getUnmetDependencies, getUsedHours, getVendorCost, getVendorSupportedOpenTasks } from "../../engine/calculations";
...
export default function PlayerApp() {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {tab === 9 && <EventInbox state={state} setState={setState} update={update} setModal={setModal} />}
...
function TaskMini({ task, update }: { task: Task; update: UpdateTeamState }) {
  ...
  const selectedBudgetOption = getSelectedBudgetOption(task);
  return <article className="task-card">
    <strong>{task.id} · {task.title.th}</strong><small>{task.title.en}</small>
    <div className="task-facts"><span>EFFORT {task.effortHours}H</span><span>DUE BY D{task.dueDay}</span><span>COST ฿{getTaskBudgetCost(task).toLocaleString()}</span></div>
    ...
    {task.budgetOptions && task.priority !== "drop" && <Field label="ระดับงบประมาณ / BUDGET PACKAGE"><select aria-label={`Budget package for priority ${task.id}`} value={selectedBudgetOption?.id} onChange={(event) => setTask((target) => { target.selectedBudgetOptionId = event.target.value; })}>{task.budgetOptions.map((option) => <option value={option.id} key={option.id}>{option.label.th} · ฿{option.cost.toLocaleString()}</option>)}</select><small>{selectedBudgetOption?.impact.th}</small>{selectedBudgetOption?.attendanceCapacity && <small> · รองรับ {selectedBudgetOption.attendanceCapacity} คน</small>}</Field>}
...
function Market({ state, update, goTo }: { state: TeamGameState; update: UpdateTeamState; goTo: (tab: number) => void }) {
  ...
  return <>
    ...
    <div className="card-grid vendor-grid">{vendorCards.map(({ vendor, openTasks }) => <Panel key={vendor.id} title={`${vendor.id} · ${vendor.category}`} className={openTasks.length ? "vendor-recommended" : ""}>
      ...
      <strong className="vendor-price">฿{getVendorCost(state, vendor).toLocaleString()} {getVendorCost(state, vendor) < vendor.cost && <small><s>฿{vendor.cost.toLocaleString()}</s> · EVENT OFFER</small>}</strong><p>{vendor.benefit.th}</p><small>COORDINATION: {vendor.coordination}</small>
...
function EventInbox({ state, setState, update, setModal }: { state: TeamGameState; setState: React.Dispatch<React.SetStateAction<TeamGameState>>; update: UpdateTeamState; setModal: (value: string) => void }) {
  ...
  const attendanceGap = state.confirmedAttendance === undefined ? undefined : Math.max(130 - state.confirmedAttendance, 0);
  return <><h1>EVENT INBOX / รับเหตุการณ์</h1><Panel title="EVENT CODE / รหัสเหตุการณ์"><div className="event-entry"><input aria-label="รหัสเหตุการณ์จาก Facilitator" placeholder="กรอกรหัสเหตุการณ์ที่ได้รับจาก Facilitator" value={code} maxLength={3} onChange={(event) => setCode(event.target.value.toUpperCase())} /><PixelButton onClick={submit}><span className="button-label-stack"><span>RECEIVE EVENT</span><span>รับเหตุการณ์</span></span></PixelButton></div>{message && <p className="warning">{message}</p>}</Panel>
    {state.appliedEventCodes.length > 0 && <Panel title="CURRENT EVENT IMPACT / ผลกระทบปัจจุบัน"><div className="event-impact-grid"><span>วงเงินปัจจุบัน <strong>฿{state.budgetCeiling.toLocaleString()}</strong></span><span>Forecast ผู้เข้าร่วม <strong>{state.expectedAttendance} คน</strong></span>{state.confirmedAttendance !== undefined && <span>ยืนยันแล้ว <strong>{state.confirmedAttendance} คน · ขาดอีก {attendanceGap}</strong></span>}<span>Network <strong>{state.networkRisk ? "AT RISK" : "ปกติ"}</strong></span>{state.cateringCutoffDay && <span>Catering Cut-off <strong>D{state.cateringCutoffDay} · เพิ่มภายหลังได้ {Math.round((state.postCutoffIncreaseLimit ?? 0) * 100)}%</strong></span>}</div></Panel>}
    {state.appliedEventCodes.includes("E03") && <BudgetPanel state={state} update={update} />}
    {state.appliedEventCodes.map((eventCode) => { const event = events.find((item) => item.code === eventCode)!; return <Panel key={eventCode} title={`${eventCode} · ${event.title.th} / ${event.title.en}`}><Badge tone={event.type === "problem" ? "orange" : event.type === "change" ? "purple" : "green"}>{event.type.toUpperCase()}</Badge><p>{event.announcement.th}</p><p>{event.announcement.en}</p><p className="callout"><strong>ผลต่อแผน:</strong> {event.effect.th}</p><p><strong>ทีมควรทบทวน:</strong> {event.notice.th}</p><Field label="TEAM RESPONSE / ทีมจะตอบสนองอย่างไร"><textarea rows={4} placeholder="ผลกระทบต่อ Outcome/แผน, สิ่งที่จะเปลี่ยน, Owner และสิ่งที่ต้องตัดสินใจ..." value={state.eventNotes[eventCode] ?? ""} onChange={(change) => update((next) => { next.eventNotes[eventCode] = change.target.value; })} /></Field></Panel>; })}</>;
}
